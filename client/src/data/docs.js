...
const docs = [
  // ━━━ objects ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    ...
    signature: "sphere(pos=vector(0,0,0), radius=0.5, color=color.white)",
    params: [
      { name: 'pos',         type: 'vector',  default: 'vector(0,0,0)', desc: { ko: '위치', en: 'Position' } },
      { name: 'radius',      type: 'number',  default: '0.5',          desc: { ko: '반지름', en: 'Radius' } },
...
cube = box(pos=vector(0, 0.5, 0), size=vector(1, 1, 1), color=color.blue)`,
  ...
  },
  {
    ...
    signature: "cylinder(pos=vector(0,0,0), axis=vector(1,0,0), radius=0.5, color=color.white)",
    params: [
...
vel = arrow(pos=ball.pos, axis=vector(2, 1, 0), shaftwidth=0.08, color=color.yellow)`,
  ...
  },
  {
    ...
    signature: "cone(pos=vector(0,0,0), axis=vector(1,0,0), radius=0.5, color=color.white)",
    params: [
      { name: 'pos',     type: 'vector',  default: 'vector(0,0,0)', desc: { ko: '밑면 중심 위치', en: 'Base center position' } },
      { name: 'axis',    type: 'vector',  default: 'vector(1,0,0)', desc: { ko: '꼭짓점 방향과 높이', en: 'Apex direction and height' } },
      { name: 'radius',  type: 'number',  default: '0.5',          desc: { ko: '밑면 반지름', en: 'Base radius' } },
      { name: 'color',   type: 'vector',  default: 'color.white',  desc: { ko: '색상', en: 'Color' } },
      { name: 'opacity', type: 'number',  default: '1.0',          desc: { ko: '불투명도 (0~1)', en: 'Opacity (0–1)' } },
      { name: 'visible', type: 'boolean', default: 'True',         desc: { ko: '화면 표시 여부', en: 'Visibility' } },
    ],
...
print(hat(v))    # <0, 1, 0> (동일)`,
  ...
  // ━━━ scene ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    ...
    description: {
      ko: '초당 프레임 수를 제어합니다. 애니메이션 루프에서 반드시 await와 함께 사용해야 합니다.',
      en: 'Controls frame rate. Must be awaited inside animation loops.',
    },
    signature: "rate(fps)",
    params: [
...
for i in range(100):
  ...
  },
  {
    ...
    description: {
      ko: '지정한 시간(초) 동안 실행을 멈춥니다. 비동기 함수입니다.',
      en: 'Pauses execution for the given seconds. Async function.',
    },
...
box(size=vector(2, 0.5, 2), color=color.orange)`,
  ...
  // ━━━ colors ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    ...
    params: [
      ...
      { name: 'color.brown',    type: 'vector', default: 'vector(0.6,0.3,0)',       desc: { ko: '갈색', en: 'Brown' } },
      ...
      { name: 'color.skyblue',  type: 'vector', default: 'vector(0.53,0.81,0.98)',  desc: { ko: '하늘색', en: 'Sky Blue' } },
      { name: 'color.lime',     type: 'vector', default: 'vector(0,1,0)',           desc: { ko: '라임', en: 'Lime' } },
      ...
      { name: 'color.violet',   type: 'vector', default: 'vector(0.93,0.51,0.93)',  desc: { ko: '바이올렛', en: 'Violet' } },
      ...
      { name: 'color.mint',     type: 'vector', default: 'vector(0.6,1,0.6)',       desc: { ko: '민트', en: 'Mint' } },
      { name: 'color.peach',    type: 'vector', default: 'vector(1,0.9,0.71)',      desc: { ko: '복숭아', en: 'Peach' } },
      { name: 'color.lavender', type: 'vector', default: 'vector(0.9,0.9,0.98)',    desc: { ko: '라벤더', en: 'Lavender' } },
...
for i, c in enumerate(colors_list):
  ...
  },
  {
    ...
    params: [
      { name: "색상['빨강'] / 색상['빨']",   type: 'vector', default: 'color.red',     desc: { ko: '빨강', en: 'Red' } },
      ...
      { name: "색상['초록'] / 색상['초']",    type: 'vector', default: 'color.green',   desc: { ko: '초록', en: 'Green' } },
      { name: "색상['파랑'] / 색상['파']",    type: 'vector', default: 'color.blue',    desc: { ko: '파랑', en: 'Blue' } },
      { name: "색상['보라']",                type: 'vector', default: 'color.purple',  desc: { ko: '보라', en: 'Purple' } },
...
sphere(pos=vector(2, 0, 0), radius=0.5, color=vector(1, 0.4, 0.7))`,
  ...
  // ━━━ sound ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'play_sound',
    category: 'sound',
    title: { ko: '소리 재생 (play_sound)', en: 'play_sound' },
    description: {
      ko: '주파수를 지정하여 소리를 재생합니다. 파형 종류와 음량을 조절할 수 있습니다.',
      en: 'Plays a tone with the specified frequency. You can set waveform type and volume.',
    },
    signature: "play_sound(frequency=440, duration=0.3, type='sine', volume=0.3)",
    params: [
...
play_sound(440, 0.5)
await sleep(0.6)
play_sound(880, 0.5, type='square')`,
    tags: ['sound', 'audio', 'tone', 'frequency', 'beep'],
  },
  {
    id: 'play_note',
    category: 'sound',
    title: { ko: '음표 재생 (play_note)', en: 'play_note' },
    description: {
      ko: "음표 이름으로 소리를 재생합니다. 영문('C4') 또는 한글('도4') 모두 가능합니다.",
      en: "Plays a note by name. Supports English ('C4') and Korean ('도4') notation.",
    },
    signature: "play_note(name='C4', duration=0.5, type='sine', volume=0.3)",
    params: [
      { name: 'name',     type: 'string', default: "'C4'",    desc: { ko: "음표 이름 ('C4', '도4', 'C#4', '도#4')", en: "Note name ('C4', '도4', 'C#4', '도#4')" } },
...
for n in notes:
    play_note(n, 0.3)
    await sleep(0.35)`,
    tags: ['sound', 'audio', 'note', 'music', 'melody', 'korean', '도레미'],
  },
  {
    id: 'play_chord',
    category: 'sound',
    title: { ko: '화음 재생 (play_chord)', en: 'play_chord' },
    description: {
      ko: '여러 주파수를 동시에 재생하여 화음을 만듭니다.',
      en: 'Plays multiple frequencies simultaneously to create a chord.',
    },
    signature: "play_chord(frequencies, duration=0.5, type='sine', volume=0.3)",
    params: [
    ...
    code: `from vpython import *
# C 장조 코드 (도-미-솔)
play_chord([261.63, 329.63, 392.00], duration=1.0)
await sleep(1.2)
# G 장조 코드 (솔-시-레)
play_chord([392.00, 493.88, 587.33], duration=1.0)`,
    tags: ['sound', 'audio', 'chord', 'harmony', 'music'],
  },
  {
    id: 'play_sequence',
    category: 'sound',
...
play_sequence(melody)`,
    tags: ['sound', 'audio', 'sequence', 'melody', 'music', 'song'],
  },
  {
    id: 'play_sfx',
    category: 'sound',
    title: { ko: '효과음 재생 (play_sfx)', en: 'play_sfx' },
    description: {
      ko: '내장 효과음을 재생합니다. 게임이나 인터랙티브 프로그램에 유용합니다.',
      en: 'Plays a built-in sound effect. Useful for games and interactive programs.',
    },
    signature: "play_sfx(name)",
    params: [
      { name: 'name', type: 'string', default: '-', desc: {
        ko: "효과음 이름: 'jump', 'coin', 'powerup', 'death', 'fireball', 'pipe', '1up', 'select', 'warning', 'explosion', 'laser', 'success', 'error', 'levelup'",
        en: "SFX name: 'jump', 'coin', 'powerup', 'death', 'fireball', 'pipe', '1up', 'select', 'warning', 'explosion', 'laser', 'success', 'error', 'levelup'",
      } },
    ],
    code: `from vpython import *
play_sfx('coin')
...
await sleep(1)
play_sfx('explosion')`,
    tags: ['sound', 'audio', 'sfx', 'effect', 'game', 'coin', 'jump', 'explosion'],
  },
  {
    id: 'bgm',
    category: 'sound',
    title: { ko: '배경음악 (start_bgm / stop_bgm)', en: 'start_bgm / stop_bgm' },
    description: {
      ko: '내장 배경음악을 반복 재생하거나 정지합니다.',
      en: 'Starts or stops a built-in background music loop.',
    },
    signature: "start_bgm(name)  /  stop_bgm()",
    params: [
      { name: 'name', type: 'string', default: '-', desc: {
...
await sleep(5)
stop_bgm()`,
    tags: ['sound', 'audio', 'bgm', 'music', 'background', 'loop'],
  },
  {
    id: 'note-constants',
    category: 'sound',
    title: { ko: '음계 상수 (note)', en: 'Note Constants' },
    description: {
      ko: 'note.C3 ~ note.C6 범위의 주파수 상수입니다. play_sound의 frequency에 사용합니다.',
      en: 'Frequency constants from note.C3 to note.C6. Use as the frequency argument for play_sound.',
    },
    signature: "note.C4  # 261.63 Hz",
    params: [
    ...
    code: `from vpython import *
play_sound(note.C4, 0.3)
await sleep(0.35)
play_sound(note.E4, 0.3)
await sleep(0.35)
play_sound(note.G4, 0.3)
await sleep(0.35)
play_sound(note.C5, 0.6)`,
    tags: ['sound', 'audio', 'note', 'constant', 'frequency', 'pitch'],
  },
  ...
  // ━━━ charts ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    ...
    signature: "scatter3d(points, colors=None, size=0.08, colormap='rainbow', opacity=1.0, show_axes=True)",
    params: [
...
scatter3d(points, colormap='rainbow', size=0.1)`,
  ...
  },
  {
    ...
    signature: "surface3d(z_data, x_range=None, y_range=None, colormap='viridis', wireframe=False, opacity=1.0)",
    params: [
...
surface3d(z, colormap='viridis')`,
  ...
  },
  {
    ...
    signature: "line3d(points, color=color.white, width=0.03, show_axes=True)",
    params: [
      { name: 'points',    type: 'list',    default: '-',          desc: { ko: '3D 좌표 리스트 [[x,y,z], ...]', en: '3D coordinate list [[x,y,z], ...]' } },
...
line3d(points, color=color.cyan, width=0.05)`,
  ...
  },
  {
    ...
    signature: "bar3d(values, labels=None, colors=None, colormap='plasma', bar_width=0.6, show_axes=True)",
    params: [
...
values = [3, 7, 2, 5, 9, 4]
labels = ['월', '화', '수', '목', '금', '토']
bar3d(values, labels=labels, colormap='plasma')`,
